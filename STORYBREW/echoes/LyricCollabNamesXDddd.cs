...
namespace StorybrewScripts
{
    public class LyricCollabNamesXDddd : StoryboardObjectGenerator
    {
        ...
        public void SpriteCommands(OsbSprite sprite, int StartTime, int EndTime, Color pixelColor, Vector2 location, string path)
        {
            ...
            // sprite.Fade(EndTime - (tick * 4), EndTime, 0.5, 0);
            sprite.Fade(EndTime - (tick * 4), EndTime, dotBrightness, 0);
            sprite.Color(EndTime - (tick * 4), pixelColor);
            sprite.Scale(EndTime - (tick * 4), DotScale);

            if(toggle) {
            var mainSprite = GetLayer("").CreateSprite(path, OsbOrigin.Centre);
            mainSprite.Move(StartTime, Origin);
            mainSprite.Fade(StartTime, StartTime + (tick * 2), 0, 1);
