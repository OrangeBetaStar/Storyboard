...
{
    public class LyricsOrig : StoryboardObjectGenerator
    {
        [Configurable]
                ...
                foreach (var line in subtitleLine.Text.Split('\n'))
                {
                    ...
                    foreach (var letter in line)
                    {
                        ...
                        if (!texture.IsEmpty)
                        {
                            ...
                            /*
                            var lyricsCirc = layer.CreateSprite("SB/highlight.png", OsbOrigin.Centre);
                            lyricsCirc.Scale(subtitleLine.StartTime, 0.2);
                            lyricsCirc.Fade(subtitleLine.StartTime - 200 + (countThings * tick), subtitleLine.StartTime + (countThings * tick), 0, 1);
                            lyricsCirc.Fade(subtitleLine.EndTime - 200, subtitleLine.EndTime, 1, 0);
                            */
                            ...
                            sprite.Fade(subtitleLine.StartTime - 200 + (countThings * tick), subtitleLine.StartTime + (countThings * tick), 0, 1);
                            sprite.Fade(subtitleLine.EndTime - 200, subtitleLine.EndTime, 1, 0);
                         //   lyricsCirc.Move(subtitleLine.StartTime, subtitleLine.StartTime, letterX + xFix, letterY + yFix, letterX + xFix, letterY + yFix);
