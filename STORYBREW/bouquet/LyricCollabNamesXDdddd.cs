...
{
    public class LyricCollabNamesXDdddd : StoryboardObjectGenerator
    {
        [Configurable]
        public string SubtitlesPath = "credit";
        ...
        public void SpriteCommands(OsbSprite sprite, int StartTime, int EndTime, Color pixelColor, Vector2 location, string path)
        {
            ...
            sprite.Move(OsbEasing.Out, StartTime, StartTime + (tick * 2), -OffsetXAfterEffect + location.X, -OffsetYAfterEffect + location.Y, location.X, location.Y);
