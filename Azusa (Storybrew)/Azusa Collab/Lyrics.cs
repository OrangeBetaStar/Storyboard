...
namespace StorybrewScripts
{
    ...
    {
        [Configurable]
        public string SubtitlesPath = "scriptslibrary/buttonThingNames.ass";

        [Configurable]
        public string FontName = "Fashion Fetish Light";

        [Configurable]
        public string SpritesPath = "SB/button";
        ...
        [Configurable]
        public float SubtitleX = 15;

        [Configurable]
        public float SubtitleY = 350;
        ...
        public void generatePerLine(FontGenerator font, SubtitleSet subtitles, StoryboardLayer layer, bool additive)
        {
            foreach (var line in subtitles.Lines)
            {
                ...
                sprite.Move(line.StartTime, line.StartTime, SubtitleX, SubtitleY, SubtitleX, SubtitleY);
                ...
                foreach (var line in subtitleLine.Text.Split('\n'))
                {
                    ...
                    foreach (var letter in line)
                    {
                        ...
                        if (!texture.IsEmpty)
                        {
                            ...
                            var sprite = layer.CreateSprite(texture.Path, OsbOrigin.Centre);
                            sprite.Move(subtitleLine.StartTime, subtitleLine.StartTime, SubtitleX+startLine, SubtitleY, SubtitleX+startLine, SubtitleY);
                            sprite.Scale(subtitleLine.StartTime, FontScale);
                            sprite.Fade(subtitleLine.StartTime - 200 + eachLetter, subtitleLine.StartTime + eachLetter, 0, 1);
                            sprite.Fade(subtitleLine.EndTime - 200, subtitleLine.EndTime, 1, 0);
                            startLine = startLine + 20;
                            eachLetter = eachLetter + 70;
